...
# 6. Конфігурація маршруту аналізу мови реєстрацій
resource "aws_apigatewayv2_route" "registration_language" {
  api_id    = aws_apigatewayv2_api.http_api.id
  route_key = "GET /registrations/{id}/lang"
  target    = "integrations/${aws_apigatewayv2_integration.lambda_integration.id}"
}

# 7. Надання дозволу API Gateway на виклик функції Lambda
