...
<head>
 ...
 <style>
 body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #e9ecef; color: 
#495057; text-align: center; padding-top: 10vh; }
 .container { background: white; padding: 40px; border-radius: 12px; box-shadow: 0 10px 20px 
rgba(0,0,0,0.1); display: inline-block; max-width: 600px; width: 100%; }
 h1 { color: #5c6bc0; font-size: 24px; margin-bottom: 20px; }
 p { font-size: 16px; margin: 10px 0; text-align: left; border-bottom: 1px solid #f1f3f5; padding-bottom: 
5px; }
...
# Якщо директорія не стандартна, Apache блокує доступ згідно політик безпеки
if ! grep -qF "<Directory ${DOC_ROOT}>" /etc/apache2/apache2.conf; then
